...
# asset_path: str : 资产创建路径
# unique_name: bool : 如果为True，将在资产名称的末尾添加号码，直到唯一的号码
# asset_class: obj unreal.Class : 资产类
# asset_factory: obj unreal.Factory : 类的相关工厂.
# return: obj : 创建的资产
...
# paths: List of str : Asset paths [路径：str列表：资产路径]
...
# path: str : Directory path [路径：STR：目录路径]
# return: bool : True if the operation succeeds
def createDirectory(path=''):
    return unreal.EditorAssetLibrary.make_directory(directory_path=path)


# from_dir: str : Directory path to duplicate [目录路径复制]
# to_dir: str : Duplicated directory path [重复的目录路径]
# return: bool : True if the operation succeeds 
...
# from_dir: str : Directory path to rename [目录路径重命名]
# to_dir: str : Renamed directory path [重命名的目录路径]
# return: bool : True if the operation succeeds
...
# from_path str : Asset path to duplicate [资产路径复制]
# to_path: str : Duplicated asset path [重复的资源路径]
# return: bool : True if the operation succeeds
...
# 复制资源对话框
# Note: This function will also work on assets of the type level. (But might be really slow if the level is huge) 注意：此功能也适用于类型级别的资产。（但如果水平很大，可能会很慢）
# from_path: str : Asset path to duplicate 资产路径复制
# to_path: str : Duplicate asset path 重复的资源路径
# show_dialog: bool : True if you want to show the confirm pop-up 如果你想显示确认弹出真
# return: bool : True if the operation succeeds
